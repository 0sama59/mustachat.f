...
function sendAction(targetName, type, minutes) {
    ...
    for (let [ws, nick] of clients.entries()) {
        if (nick === targetName && ws.readyState === ws.OPEN) {
...
wss.on('connection', ws => {
    ...
    ws.on('message', message => {
        ...
        if (data.type === "nick") {
            ...
            if (isBanned(newNick)) {
                ...
                ws.send(JSON.stringify({ type: "error", message: `You are banned for ${remainingMinutes} more minutes.` }));
                return;
            }

            // 2. Already Taken Check
            if (isTaken && newNick !== clients.get(ws)) {
                ws.send(JSON.stringify({ type: "error", message: `Nickname '${newNick}' is already taken!` }));
                return;
            }

            const oldNick = clients.get(ws);
        ...
        if (data.type === "chat") {
            ...
            if (isBanned(nick)) {
                const banTime = bannedUsers.get(nick);
                const remainingMinutes = Math.ceil((banTime - Date.now()) / (60 * 1000));
                const wsInstance = Array.from(clients.entries()).find(([w, n]) => n === nick)?.[0];
                if (wsInstance) {
                     wsInstance.send(JSON.stringify({ type: "chat", nick: "SYSTEM", text: `You are banned for ${remainingMinutes} more minutes.` }));
                }
                return;
            }
            ...
            if (mutedUsers.has(lowerNick)) {
                const wsInstance = Array.from(clients.entries()).find(([w, n]) => n === nick)?.[0];
                if (wsInstance) {
                    wsInstance.send(JSON.stringify({ type: "chat", nick: "SYSTEM", text: "You are currently muted and cannot send messages." }));
                }
                return;
            }
            ...
            if (isChatFrozen && lowerNick !== "nimda") {
                const wsInstance = Array.from(clients.entries()).find(([w, n]) => n === nick)?.[0];
                if (wsInstance) {
                    wsInstance.send(JSON.stringify({ type: "chat", nick: "SYSTEM", text: "The chat is currently frozen by the Administrator. Your message was not sent." }));
                }
                return;
            }
            ...
            if (lowerNick === "nimda" && text.startsWith("/")) {
                ...
                // /rename
                    ...
                    for (let [clientWs, clientNick] of clients.entries()) {
                        if (clientNick === target) {
                            clients.set(clientWs, newNick); 
                            renamed = true;
                            break;
                        }
                    }
                ...
                // /highlight
                    ...
                    wss.clients.forEach(c => { if (c.readyState === c.OPEN) c.send(highlightData); });
                }
                // /clear
                else if (cmd === "/clear") {
    ...
    ws.on('close', () => {
        const closedNick = clients.get(ws);
        clients.delete(ws);
        if (closedNick) {
